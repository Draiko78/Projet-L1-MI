...
from inputPlayer import movement
#from capteurs.lightS import lightSensor
from plante2 import plantation
from sqlDB import *
        ...
        while self.run:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                    plants_dict = extract_plants_from_map()
                    print(plants_dict)
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_UP:
                        movement(self.player, gmap, 'up')
                    if event.key == pygame.K_DOWN:
                        movement(self.player, gmap, 'down')
                    if event.key == pygame.K_RIGHT:
                        movement(self.player, gmap, 'right')
                    if event.key == pygame.K_LEFT:
                        movement(self.player, gmap, 'left')
